...
from src.attack.ACE_PGD import ACE_PGD
...
class Trainer():
    ...
    def train(self, model, tr_loader, va_loader=None, adv_train=False):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(opt, milestones=[40000, 60000],gamma=0.1)
        _iter = 0
...
#                     adv_data = self.attack.perturb(data, label, 'mean', True)
                    adv_data,_ = self.attack.perturb(data, label)
...
#                         adv_data = self.attack.perturb(data, label, 'mean', False)
    ...
    def test(self, model, loader, adv_test=True, use_pseudo_label=False):
...
#                         adv_data = self.attack.perturb(data, pred if use_pseudo_label else label, 'mean', False)
                        adv_data,_ = self.attack.perturb(data, pred if use_pseudo_label else label)
...
#                                         _type=args.perturbation_type)
    attack = ACE_PGD(model, args.steps, args.epsilon, args.alpha, max_iters=50)
    ...
    if args.todo == 'train':
        ...
        te_loader = DataLoader(torch.utils.data.Subset(te_dataset,range(0,500)), batch_size=args.batch_size, shuffle=False, num_workers=4)
        trainer.train(model, tr_loader, te_loader, args.adv_train)
#         trainer.train(model,te_loader)
        ...
        te_loader = DataLoader(torch.utils.data.Subset(te_dataset,range(500,10000)), batch_size=args.batch_size, shuffle=False, num_workers=4)
